...
function Index() {
  ...
  return (
    <main className="min-h-screen overflow-hidden bg-background text-foreground">
      <section className="relative min-h-[94svh] border-b border-border sm:min-h-[92svh]">
        <img
          src={heroPhoto}
        ...
        <div className="relative mx-auto flex min-h-[94svh] max-w-7xl items-end px-5 pb-12 sm:min-h-[92svh] sm:px-10 sm:pb-20 lg:items-center lg:px-16 lg:pb-0">
          <div className="hero-copy max-w-xl animate-fade-in">
            ...
            <h1 className="max-w-[21rem] font-hand text-5xl leading-[0.98] text-hero-ink sm:max-w-xl sm:text-7xl lg:text-8xl">
      ...
      <section className="paper-texture px-4 py-20 sm:px-8 sm:py-32" aria-labelledby="letter-title">
        <div className="reveal mx-auto max-w-3xl">
          <p className="section-kicker">Open me when you have a quiet minute</p>
          <article className="letter-paper relative mt-8 px-6 py-11 sm:mt-10 sm:px-16 sm:py-16">
            ...
            <div className="mt-7 space-y-5 font-body text-base leading-7 text-ink-soft sm:mt-8 sm:text-lg sm:leading-8">
              <p>
      ...
      <section className="bg-table py-20 sm:px-8 sm:py-32" aria-labelledby="gallery-title">
        <div className="mx-auto max-w-6xl">
          <div className="reveal px-5 text-center sm:px-0">
          ...
          <div className="gallery-scroll mt-12 flex snap-x snap-mandatory gap-5 overflow-x-auto px-[11vw] pb-10 pt-3 sm:mt-20 sm:grid sm:grid-cols-2 sm:gap-10 sm:overflow-visible sm:px-0 sm:pb-0 lg:grid-cols-4">
            {gallery.map((photo, index) => (
              <button
                key={photo.caption}
                type="button"
                onClick={() => setSelectedPhoto(photo)}
                className={`reveal polaroid polaroid-${index + 1} group min-w-[72vw] snap-center cursor-zoom-in text-left sm:min-w-0`}
      ...
      <section className="paper-texture px-5 py-20 sm:px-8 sm:py-32" aria-labelledby="memories-title">
        <div className="mx-auto max-w-5xl">
          ...
          <div className="mt-14 divide-y divide-border border-y border-border">
            {memories.map((memory, index) => (
              <article key={memory.title} className="reveal grid grid-cols-[6.5rem_minmax(0,1fr)] items-center gap-5 py-7 sm:grid-cols-[9rem_1fr] sm:gap-10 sm:py-8">
                <img
                  src={memory.src}
                  alt=""
                  loading="lazy"
                  width={1000}
                  height={1000}
                  className={`aspect-square w-full object-cover shadow-photo ${index === 1 ? "rotate-2" : "-rotate-2"}`}
                />
                <div>
                  <p className="font-hand text-3xl text-accent-strong">{String(index + 1).padStart(2, "0")}</p>
                  <h3 className="mt-1 font-hand text-3xl leading-none text-foreground sm:text-4xl">{memory.title}</h3>
                  <p className="mt-3 max-w-2xl font-body text-sm leading-6 text-ink-soft sm:text-base sm:leading-7">{memory.text}</p>
                </div>
              </article>
            ))}
          </div>
      ...
      <footer className="relative bg-footer px-5 py-20 text-center text-footer-foreground sm:py-32">
        <div className="reveal mx-auto max-w-3xl">
      ...
      {selectedPhoto && (
        <div className="fixed inset-0 z-50 grid place-items-center bg-modal-backdrop p-5" role="dialog" aria-modal="true" aria-label={selectedPhoto.caption} onClick={() => setSelectedPhoto(null)}>
          <button type="button" onClick={() => setSelectedPhoto(null)} className="absolute right-4 top-4 grid size-12 place-items-center rounded-full bg-background text-foreground shadow-photo sm:right-5 sm:top-5 sm:size-11" aria-label="Close photo">
